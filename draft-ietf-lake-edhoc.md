...
Symmetric algorithms used in EDHOC such as SHA-256 and AES-CCM-16-64-128 are practically secure against even large quantum computers. Two of NIST security levels for quantum-resistant public-key cryptography are based on AES-128 and SHA-256. Quantum computer will likely be expensive, slow (MHz) due to heavy error correction, and Grover’s algorithm, which is proven to be optimal, cannot effectively be parallelized. Grover’s algorithm will provide little or no advantage in attacking AES, and AES-128 will remain secure for decades to come {{NISTPQC}}.
