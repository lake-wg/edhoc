...
* Similarly, when a PKI is used with CWTs, each party needs to have a trusted self-signed CWT or raw public key to verify the CWTs, and a specific identity or set of identities in the `sub`(subject) claim of the CWT to determine if it is allowed to continue the EDHOC protocol.
