...
An example of CRED_x being a UCCS in bytewise lexicographic order containing an X25519 static Diffie-Hellman key and where the parties have agreed on an EUI-64 identity is shown below:
...
After receiving SUITES_R, the Initiator can determine which cipher suite to select for the next EDHOC session with the Responder.
