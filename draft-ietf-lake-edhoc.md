...
Certain Internet of Things (IoT) deployments require technologies which are highly performant in constrained environments {{RFC7228}}. IoT devices may be constrained in various ways, including memory, storage, processing capacity and power. The connectivity for these settings may also exhibit constraints such as unreliable and lossy channels, highly restricted bandwidth and dynamic topology. The IETF has acknowledged this problem by standardizing a range of lightweight protocols and enablers designed for the IoT, including the Constrained Application Protocol (CoAP, {{RFC7252}}), Concise Binary Object Representation (CBOR, {{RFC7049}}), and Static Context Header Compression (SCHC, {{RFC8724}}).

The need for special protocols targeting constrained IoT deployments extends also to the security domain {{I-D.ietf-lake-reqs}}. Important characteristics in constrained environments are the number of round trips and protocol message sizes, which if kept low can contribute to good performance by enabling transport over a small number of radio frames, avoiding latency due to duty cycles, etc. Another important criteria is code size, which may be prohibitive for certain deployments due to device characteristics or network load during firmware update. Additionally, security at the application layer provides an attractive option for IoT deployments when the solution needs to support a variety of underlying transport technologies, potentially even within a single connection.

Some security solutions for this setting exist already. CBOR Object Signing and Encryption (COSE) {{RFC8152}}) specifies basic application layer security services efficiently encoded in CBOR. Another example is Object Security for Constrained RESTful Environments (OSCORE) {{RFC8613}} which is a lightweight communication security extension to CoAP using CBOR and COSE. In order to establish good quality cryptographic keys for security protocols such as COSE and OSCORE, the two endpoints may run an Elliptic Curve Diffie-Hellman (ECDH) key exchange protocol with ephemeral keys, from which shared secret key material can be derived. Such a key exchange protocol should also be lightweight; to prevent bad performance in case of repeated use, e.g., due to device rebooting or frequent rekeying for security reasons; or to avoid latencies in a network formation setting with many devices authenticating at the same time.

This document specifies Ephemeral Diffie-Hellman Over COSE (EDHOC), a lightweight authenticated key exchange protocol providing good security properties including perfect forward secrecy, identity protection, and cipher suite negotation. Authentication can be based on raw public keys (RPK) or public key certificates, and requires the application to provide input on how verify that endpoints are trusted. This specificaton focuses on referencing instead of transporting credentials to reduce message overhead.

After successful completion of the EDHOC protocol, application keys and other application specific data can be derived using the EDHOC-Exporter interface, and lightweight rekeying with forward secrecy is provided by the EDHOC-Rekey-FS interface. A main use case for EDHOC is to establish a security context for OSCORE. EDHOC is built on the same lightweight primitives as OSCORE: CBOR for encoding, COSE for cryptography, and CoAP for transport. By reusing existing libraries the additional code size can be kept very low. EDHOC is not bound to a particular transport, but it is recommended to transfer EDHOC messages in CoAP payloads.

The authenticated key exchange and the key derivation makes use of known protocol constructions, such as {{SIGMA}} and Extract-and-Expand {{RFC5869}}, implemented in terms of CBOR and COSE. COSE also provides crypto agility and enables use of future algorithms and headers designed for constrained IoT.
...
A typical setting is when one of the endpoints is constrained or in a constrained network, and the other endpoint is a node on the Internet (such as a mobile phone) or at the edge of the constrained network (such as a gateway). Thing-to-thing interactions over constrained networks are also relevant since both endpoints would then benefit from the lightweight properties of the protocol. EDHOC could e.g. be run when a device/device(s) connect(s) for the first time, or to establish fresh keys which are not leaked by a later compromise of the long-term keys. Further security properties are described in {{sec-prop}}.
