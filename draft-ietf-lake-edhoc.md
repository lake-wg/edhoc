...
The need for special protocols targeting constrained IoT deployments extends also to the security domain {{I-D.ietf-lake-reqs}}. Important characteristics in constrained environments are round trips and protocol message sizes, which can enable transport over a small number of radio frames, avoiding latency due to duty cycles, etc. Another important criteria is code size, which may be prohibitive for certain deployments due to device characteristics or firmware update performance. Additionally, security at the application layer provides an attractive option for IoT deployments when there is a variety of underlying transport technologies to support, potentially even within a single connection. 

Some security solutions for this setting exist already. CBOR Object Signing and Encryption (COSE) {{RFC8152}}) specifies basic application layer security services efficienty encoded in CBOR. Another example is Object Security for Constrained RESTful Environments (OSCORE) {{RFC8613}} which is a lightweight communication security extension to CoAP using CBOR and COSE. In order to establish good quality cryptographic keys for security protocols such as COSE and OSCORE, the two endpoints may run an Elliptic Curve Diffie-Hellman (ECDH) key exchange protocol with ephemeral keys, from which shared secret key material can be derived. Such a key exchange protocol should also be lightweight; to enable repeated use, e.g. due to device rebooting or frequent rekeying for security reasons, or to avoid latencies in a network formation setting with many devices joining at the same time. 
...
The authenticated key exchange and the key derivation makes use of known protocol constructions, such as {{SIGMA}}, NIST SP-800-56A {{SP-800-56A}}, and Extract-and-Expand {{RFC5869}}, implemented in terms of CBOR and COSE. COSE also provides crypto agility and enables use of future algorithms and headers designed for constrained IoT.
