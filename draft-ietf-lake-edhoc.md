...
Certain Internet of Things (IoT) deployments require technologies which are performant in constrained environments {{RFC7228}}. IoT devices may be constrained in various ways, including memory, storage, processing capacity and power. The connectivity for these settings may also exhibit constraints such as unreliable and lossy channels, highly restricted bandwidth and dynamic topology. The IETF has acknowledged this problem by standardizing a range of lightweight protocols and enablers designed for the IoT including the Constrained Application Protocol (CoAP, {{RFC7252}}), Concise Binary Object Representation (CBOR, {{RFC7049}}), and Static Context Header Compression (SCHC, {{RFC8724}}).

The need for special protocols targeting constrained IoT deployments extends also to the security domain {{I-D.ietf-lake-reqs}}). Important characteristics in constrained environments are round trips and protocol message size, which enables transport over a small number of radio frames, avoids latency due to duty cycles, etc. Another important criteria is code size which may be prohibitive for certain deployments due to device characteristics or firmware update performance. Finally, security at the application layer provides an attractive option for IoT deployments when there is a variety of underlying transport technologies to support, potentially even in a single connection. 

Some solutions exist already. CBOR Object Signing and Encryption (COSE) {{RFC8152}}) specifies basic application layer security services efficienty encoded in CBOR. Another example is Object Security for Constrained RESTful Environments (OSCORE) {{RFC8613}} which is a lightweight communication security extension to CoAP using CBOR and COSE. In order to establish good quality cryptographic keys for security protocols such as COSE and OSCORE, the two endpoints can run an Elliptic Curve Diffie-Hellman (ECDH) key exchange protocol with ephemeral keys, from which shared secret key material can be derived. The key exchange protocol should also be lightweight to enable repeated use, e.g. due to device rebooting or frequent rekeying for security reasons, or in a network bootstrapping setting when a large number of devices join over common links in a short period of time resulting in large latencies. 
...
The ECDH exchange and the key derivation makes use of known protocol constructions, such as {{SIGMA}}, NIST SP-800-56A {{SP-800-56A}}, and Extract-and-Expand {{RFC5869}}, implemented in terms of CBOR and COSE. COSE also provides crypto agility and enables use of future algorithms and headers designed for constrained IoT.
