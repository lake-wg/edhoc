...
void test_vectors( EDHOCKeyType type_I, EDHOCKeyType type_R, EDHOCCorrelation corr, EDHOCSuite selected_suite,
    ...
    int index = 100 * ( 25 * METHOD_CORR + 5 * attr_I + attr_R ) + selected_suite;
    srand( index );
...
//        SUITES_I = cbor_arr( 3 ) + cbor( selected_suite ) + cbor( 0 ) + cbor( 2 ); // One of several possible trucations of preferred suites
   ...
   // message_3 ////////////////////////////////////////////////////////////////////////////
    ...
    if ( type_R == sig ) {
        ...
        print( "PK_R (Responders's public authentication key)", PK_R );
    }
    if ( type_R == sdh ) {
    ...
    if ( full_output == true ) {
        ...
        print( "info for KEYSTREAM_2 (CBOR-encoded)", info_KEYSTREAM_2 );   
        print( "KEYSTREAM_2", KEYSTREAM_2 );
    ...
    if ( type_I == sig ) {
        ...
        print( "PK_I (Responders's public authentication key)", PK_I );
    }
    if ( type_I == sdh ) {
    ...
    print( "KeyUpdate Nonce", nonce );
    print( "PRK_4x3m  after KeyUpdate", PRK_4x3m_new );   
    print( "OSCORE Master Secret after KeyUpdate", OSCORE_secretFS );
    print( "OSCORE Master Salt after KeyUpdate", OSCORE_saltFS ); 
}
...
int main( void ) {
    ...
    // Mixed key types
    test_vectors( sig, sdh, corr_12,   suite_0, x5t,     kid,   false, false, false, true );
    test_vectors( sdh, sig, corr_12,   suite_0, kid,     x5t,   false, false, false, true );

    // Other header attributes for sig and sdh
    test_vectors( sig, sig, corr_12,   suite_0, x5u,     x5u,   false, false, false, true );
    test_vectors( sig, sig, corr_12,   suite_0, x5chain, x5bag, false, false, false, true );
    test_vectors( sdh, sdh, corr_12,   suite_0, x5chain, x5u,   false, false, false, true );
    test_vectors( sdh, sdh, corr_12,   suite_0, x5t,     x5bag, false, false, false, true );

    // Cipher suite nr. 1 and non-compressed SUITES_I
    test_vectors( sig, sig, corr_12,   suite_1, x5t,     x5t,   false, false, false, true );
    test_vectors( sdh, sdh, corr_12,   suite_1, kid,     kid,   false, false, false, true );

    // All other correlations
    test_vectors( sdh, sdh, corr_none, suite_0, kid,     kid,   false, false, false, true );
    test_vectors( sdh, sdh, corr_23,   suite_0, kid,     kid,   false, false, false, true );
    test_vectors( sdh, sdh, corr_123,  suite_0, kid,     kid,   false, false, false, true );

    // Auxileary data
    test_vectors( sdh, sdh, corr_12,   suite_0, kid,     kid,   true, false, false, true );

    // Subject names
    test_vectors( sdh, sdh, corr_12,   suite_0, kid,     kid,   false, true, false, true );

    // Long non-compressed bstr_identifiers
    test_vectors( sdh, sdh, corr_12,   suite_0, kid,     kid,   false, false, true, true );
}
